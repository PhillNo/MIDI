...
tests:
	...
	mkdir extras/jobs/encoded_files || true
	for file in $$(find extras/jobs -maxdepth 1 -type f -name \*.sh | sort); do echo -n $$file >> extras/tmp.txt; echo -n " " >> extras/tmp.txt; $$file >> extras/tmp.txt; done
